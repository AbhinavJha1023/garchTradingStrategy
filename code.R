...
eurusd <- read.csv(file="C:/Users/ABHINAV/Desktop/TimeSeries/TradingStrategy/SP500.csv",
                   header = T)
...
for (i in 0:forecasts.length) {
  ...
  if (is(fit, "warning")) {
    ...
    #we need the predictions: +1 (positive return) 
    #or -1 (negative return) / x[1] is the return itself
...
forecasts
#we use the first window.length data to fit the model
#we need the date for the forecasted values
